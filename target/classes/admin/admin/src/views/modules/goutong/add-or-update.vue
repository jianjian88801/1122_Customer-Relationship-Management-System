    ...
    <div class="addEdit-block">
        <el-form
            ...
            <input id="kehuId" name="kehuId" type="hidden">
               <el-col :span="12">
                   <el-form-item class="input" v-if="type!='info'"  label="沟通标题" prop="goutongName">
                       <el-input v-model="ruleForm.goutongName"
                                 placeholder="沟通标题" clearable  :readonly="ro.goutongName"></el-input>
                   </el-form-item>
                   <div v-else>
                       <el-form-item class="input" label="沟通标题" prop="goutongName">
                           <el-input v-model="ruleForm.goutongName"
                                     placeholder="沟通标题" readonly></el-input>
                       </el-form-item>
                   </div>
               </el-col>
                <el-col :span="12">
                    <el-form-item class="select" v-if="type!='info'"  label="沟通类型" prop="goutongTypes">
                        <el-select v-model="ruleForm.goutongTypes" :disabled="ro.goutongTypes" placeholder="请选择沟通类型">
                            <el-option
                                v-for="(item,index) in goutongTypesOptions"
                    ...
                    <div v-else>
                        <el-form-item class="input" label="沟通类型" prop="goutongValue">
                        <el-input v-model="ruleForm.goutongValue"
                            placeholder="沟通类型" readonly></el-input>
                        </el-form-item>
                    </div>
                </el-col>
                <el-col :span="24">
                    <el-form-item v-if="type!='info'"  class="input" label="沟通时间" prop="goutongTime">
                        <el-date-picker
                                value-format="yyyy-MM-dd HH:mm:ss"
                                v-model="ruleForm.goutongTime"
                                type="datetime"
                                placeholder="沟通时间"
                                :disabled="ro.goutongTime">
                        </el-date-picker>
                    </el-form-item>
                    <div v-else>
                        <el-form-item v-if="ruleForm.goutongTime" label="沟通时间" prop="goutongTime">
                            <span v-html="ruleForm.goutongTime"></span>
                        </el-form-item>
                    </div>
                </el-col>

                <el-col :span="24">
                    <el-form-item v-if="type!='info'"  label="沟通详情" prop="goutongContent">
                        <editor style="min-width: 200px; max-width: 600px;"
                                v-model="ruleForm.goutongContent"
                                class="editor"
                                action="file/upload">
                        </editor>
                    </el-form-item>
                    <div v-else>
                        <el-form-item v-if="ruleForm.goutongContent" label="沟通详情" prop="goutongContent">
                            <span v-html="ruleForm.goutongContent"></span>
                        </el-form-item>
                    </div>
                </el-col>
            </el-row>
...
<script>
    ...
    export default {
        data() {
            return {
                ...
                rules: {
                   ...
                   goutongName: [
                              { required: true, message: '沟通标题不能为空', trigger: 'blur' },
                          ],
                   goutongTypes: [
                              { required: true, message: '沟通类型不能为空', trigger: 'blur' },
                              {  pattern: /^[1-9][0-9]*$/,
                                  message: '只允许输入整数',
                                  trigger: 'blur'
                              }
                   ...
                   goutongContent: [
                              { required: true, message: '沟通详情不能为空', trigger: 'blur' },
                          ],
                   insertTime: [
        ...
        created() {
            ...
            //获取下拉框信息
                this.$http({
                    url:`dictionary/page?page=1&limit=100&sort=&order=&dicCode=goutong_types`,
                    method: "get"
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.goutongTypesOptions = data.data.list;
                    }
                });
        ...
        methods: {
            ...
            onSubmit() {
                this.$refs["ruleForm"].validate(valid => {
                    if (valid) {
                        this.$http({
                            url:`goutong/${!this.ruleForm.id ? "save" : "update"}`,
                            method: "post",
                            data: this.ruleForm
                        }).then(({ data }) => {
                            if (data && data.code === 0) {
                                this.$message({
                                    ...
                                    onClose: () => {
                                        ...
                                        this.parent.goutongCrossAddOrUpdateFlag = false;
                                        this.parent.search();
